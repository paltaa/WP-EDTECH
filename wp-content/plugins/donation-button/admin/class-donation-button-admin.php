...
class Donation_Button_Admin {
    ...
    public function custom_donationgoal_columns($column, $post_id) {
        ...
        switch ($column) {
            ...
            case 'donation_button_short_code' :
                echo $this->create_donation_goal_shoert_code( $post_id );
                break;
            default :
                break;
        }
    ...
    public function create_donation_goal_shoert_code( $id ){
        
        $goal_array_data = get_post_meta($id, 'donation_button_detail');
        $goal_array = $goal_array_data[0];
        
        $t_amount = $goal_array['donation_button_target_amount'];
        $start_date = date("Y-m-d", strtotime($goal_array['donation_button_start_date']));
        $end_date = date("Y-m-d", strtotime($goal_array['donation_button_end_date'])); 
